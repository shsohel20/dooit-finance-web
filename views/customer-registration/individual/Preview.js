      ...
      <div className="space-y-4 mt-4">
        <div className="">
          ...
          <div className="grid grid-cols-6 gap-2 py-6">
            <LabelDetails
              label="First Name"
              value={customerRegisterData.customer_details?.given_name}
            />
            <LabelDetails
              label="Middle Name"
              value={customerRegisterData?.customer_details?.middle_name}
            />
            <LabelDetails
              label="Last Name"
              value={customerRegisterData?.customer_details?.surname}
            />
            <LabelDetails
              label="Date of Birth"
              value={customerRegisterData?.customer_details?.date_of_birth}
            />
            <LabelDetails
              label="Occupation"
              value={customerRegisterData?.employment_details?.occupation}
            />
            <LabelDetails label="Email" value={customerRegisterData?.contact_details?.email} />
            <LabelDetails
              label="Industry"
              value={customerRegisterData?.employment_details?.industry}
            />
            <LabelDetails
              label="Employer' Name"
              value={customerRegisterData?.employment_details?.employer_name}
            />
            <LabelDetails
              label="Residential Address"
              value={customerRegisterData?.residential_address?.address}
            />
            <LabelDetails
              label="Country"
              value={customerRegisterData?.residential_address?.country?.value}
            />
            <LabelDetails label="State" value={customerRegisterData?.residential_address?.state} />
            <LabelDetails
              label="Zip Code"
              value={customerRegisterData?.residential_address?.postcode}
            />
          </div>
        </div>
        <div className="">
          ...
          <div className="">
            <p>
              Document Type:{" "}
              <span className="font-bold">{customerRegisterData?.document_type?.label}</span>
            </p>
            <div className="flex py-4 gap-2">
              {customerRegisterData?.documents?.map((document, index) => (
                <div
                  key={document.url + index}
                  className={cn("h-[200px] aspect-3/4 rounded-md overflow-hidden border")}
                >
                  <img
                    src={document.url}
                    alt={document.name}
        ...
        <div>
          ...
          <div className="grid grid-cols-6 gap-2 py-6">
            ...
            <LabelDetails
              ...
              value={customerRegisterData?.funds_wealth?.account_purpose}
            />
            <LabelDetails
              label="Estimated Trading Volume"
              value={customerRegisterData?.funds_wealth?.estimated_trading_volume}
            />
            <LabelDetails label="Email" value={customerRegisterData?.contact_details?.email} />
            <LabelDetails
              label="Occupation"
        ...
        <div>
          ...
          <div className="space-y-4">
            <div>
              Are you acting as a sole trader?:{" "}

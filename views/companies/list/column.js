...
export const companiesColumns = (handleView) => {
  return [
...

    {
      ...
      cell: ({ row }) => {
        return <div className="text-xs">{row.original.general_information?.local_agent?.name}</div>;
      },
    },
    {
      id: "annual-income",
      ...
      cell: ({ row }) => {
        return (
          <div className="text-xs text-muted-foreground text-wrap max-w-[300px]">
            {row.original.general_information?.registered_address?.street},{" "}
            {row.original.general_information?.registered_address?.suburb},{" "}
            {row.original.general_information?.registered_address?.state},{" "}
            {row.original.general_information?.registered_address?.postcode},{" "}
            {row.original.general_information?.registered_address?.country}
          </div>
        );
      },
    },

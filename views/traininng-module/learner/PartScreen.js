...
import {
  ...
  ArrowLeft, Lock, CheckCircle, Loader2,
} from "lucide-react";
...
export default function PartScreen({ partId, moduleId }) {
  ...
  useEffect(() => { partDataRef.current = partData; }, [partData]);
  ...
  // ── Progress update ────────────────────────────────────────────
  const sendProgress = useCallback(async (secs) => {
    const pData = partDataRef.current;
    if (!pData) return;
    const watched = secs
      ?? (isYT ? ytPlayer.current?.getCurrentTime?.() : videoRef.current?.currentTime)
      ?? 0;
    ...
    if (res?.data) {
      const dur = duration
        || (isYT ? ytPlayer.current?.getDuration?.() : videoRef.current?.duration)
        || 0;
      if (dur) setWatchedPercent(Math.min(Math.floor((res.data.watchedSeconds / dur) * 100), 100));
    }
  }, [moduleId, partId, duration, isYT]);

  const debouncedProgress = useRef(_.throttle((s) => sendProgress(s), 5000, { leading: false, trailing: true })).current;
      ...
      const player = new YT.Player(ytDiv.current, {
        ...
        events: {
          ...
          onStateChange(e) {
            ...
            if (e.data === S.PLAYING)   { setPlaying(true);  setIsBuffering(false); }
            if (e.data === S.PAUSED)    { setPlaying(false); sendProgress(); }
            if (e.data === S.BUFFERING) { setIsBuffering(true); }
            if (e.data === S.ENDED)     {
              setPlaying(false);
  ...
  const onNativePlay  = async () => {
    ...
    if (!hasStarted) { await startWatchingVideo(moduleId); setHasStarted(true); }
  };
  const onNativePause  = () => { setPlaying(false); sendProgress(); };
  const onNativeEnded  = () => { setPlaying(false); sendProgress(duration); };
  ...
  const togglePlay = useCallback(async () => {
    ...
    if (isYT) {
      ...
      if (playing) {
        ...
        p.playVideo();
        flashCenter("play");
        if (!hasStarted) { startWatchingVideo(moduleId); setHasStarted(true); }
      }
    } else {
      const v = videoRef.current;
      if (!v) return;
  ...
  const setSpeed = (s) => {
    setPlaybackRate(s); setShowSpeed(false);
  ...
  const resetTimer = () => {
    ...
    if (playing)
      controlsTimer.current = setTimeout(() => { setShowControls(false); setShowSpeed(false); }, 3000);
  };
  useEffect(() => {
    if (!playing) { setShowControls(true); clearTimeout(controlsTimer.current); }
  ...
  const goToQuiz = () =>
    router.push(`/dashboard/client/knowledge-hub/training-hub/learner/training/${moduleId}/part/${partId}/quiz`);

  // ── Render ────────────────────────────────────────────────────
  return (
    <div className="max-w-5xl mx-auto px-1 pb-10 space-y-5">
      ...
      <div className="flex items-center justify-between pt-1">
        ...
        <div className={cn(
          ...
          {quizUnlocked ? <CheckCircle className="w-3.5 h-3.5" /> : <Lock className="w-3.5 h-3.5" />}
          {quizUnlocked ? "Quiz Unlocked" : watchedPercent > 0 ? `${watchedPercent}% watched` : "Quiz Locked"}
        </div>
      </div>
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Watch at least <span className="text-foreground font-medium">{QUIZ_UNLOCK_THRESHOLD}%</span> of the video to unlock the quiz
        </p>
      </div>
      ...
      >
        ...
        {isYT && <div ref={ytDiv} className="absolute inset-0 w-full h-full" />}
        ...
        {/* Center click flash */}
        {centerFlash && (
          <div key={centerFlash.id} className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
            <div className="bg-black/50 rounded-full p-5 animate-ping" style={{ animationIterationCount: 1, animationDuration: "0.55s" }}>
              {centerFlash.type === "play"     && <Play        className="w-9 h-9 text-white fill-white" />}
              {centerFlash.type === "pause"    && <Pause       className="w-9 h-9 text-white fill-white" />}
              {centerFlash.type === "forward"  && <div className="flex items-center gap-1 text-white font-bold text-sm"><SkipForward className="w-8 h-8 fill-white" />+10s</div>}
              {centerFlash.type === "backward" && <div className="flex items-center gap-1 text-white font-bold text-sm"><SkipBack    className="w-8 h-8 fill-white" />-10s</div>}
            </div>
          </div>
        )}
        ...
        {/* Controls overlay */}
        <div
          className={cn("absolute inset-0 z-20 transition-opacity duration-300", showControls ? "opacity-100" : "opacity-0 pointer-events-none")}
          onClick={togglePlay}
        >
          <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/5 to-transparent pointer-events-none" />

          {/* Controls panel */}
          <div className="absolute bottom-0 left-0 right-0 px-5 pb-4 pt-12 space-y-2.5" onClick={(e) => e.stopPropagation()}>

            {/* Seek bar */}
            <div className="flex items-center gap-3">
              <span className="text-white/60 text-xs font-mono tabular-nums shrink-0 w-10 text-right">{formatTime(currentTime)}</span>
              <div
                className="relative flex-1 h-1 rounded-full cursor-pointer group/seek hover:h-1.5 transition-all duration-150"
                style={{ backgroundColor: "rgba(255,255,255,0.15)" }}
                onClick={handleSeek}
              >
                <div className="absolute inset-y-0 left-0 rounded-full" style={{ width: `${bufferedPct}%`, backgroundColor: "rgba(255,255,255,0.2)" }} />
                <div className="absolute inset-y-0 left-0 bg-primary rounded-full transition-[width] duration-150" style={{ width: `${playedPct}%` }} />
                <div className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-3.5 h-3.5 bg-white rounded-full shadow-lg opacity-0 group-hover/seek:opacity-100 transition-opacity pointer-events-none" style={{ left: `${playedPct}%` }} />
              </div>
              <span className="text-white/40 text-xs font-mono tabular-nums shrink-0 w-10">{formatTime(duration)}</span>
            </div>

            {/* Button row */}
            <div className="flex items-center gap-0.5">
              <button onClick={() => skip(-10)} className="p-2 text-white/70 hover:text-white hover:bg-white/10 rounded-lg transition-colors" title="Rewind 10s (←)">
                <SkipBack className="w-5 h-5" />
              </button>
              <button onClick={togglePlay} className="p-2 text-white hover:bg-white/10 rounded-xl transition-colors" title="Play/Pause (Space)">
                {playing ? <Pause className="w-6 h-6 fill-white" /> : <Play className="w-6 h-6 fill-white" />}
              </button>
              <button onClick={() => skip(10)} className="p-2 text-white/70 hover:text-white hover:bg-white/10 rounded-lg transition-colors" title="Skip 10s (→)">
                <SkipForward className="w-5 h-5" />
              </button>

              {/* Volume */}
              <div className="flex items-center gap-1 ml-1 group/vol">
                <button onClick={toggleMute} className="p-2 text-white/70 hover:text-white hover:bg-white/10 rounded-lg transition-colors">
                  <VolumeIcon className="w-5 h-5" />
                </button>
                <div className="w-0 overflow-hidden group-hover/vol:w-20 transition-all duration-200">
                  <input type="range" min="0" max="1" step="0.05" value={isMuted ? 0 : volume} onChange={handleVolume} className="w-20 h-1 cursor-pointer accent-white" />
                </div>
              </div>
              ...
              <div className="relative">
                ...
                {showSpeed && (
                  <div className="absolute bottom-full right-0 mb-2 bg-zinc-900/95 backdrop-blur-md border border-white/10 rounded-xl overflow-hidden shadow-2xl z-30 min-w-[120px]">
                    <p className="text-white/35 text-[10px] font-semibold uppercase tracking-widest px-3 pt-2.5 pb-1">Playback Speed</p>
                    {PLAYBACK_SPEEDS.map((s) => (
                      ...
                      >
                        ...
                        {playbackRate === s && s !== 1 && <div className="w-1.5 h-1.5 rounded-full bg-primary" />}
                      </button>
                    ))}
              ...
              <button onClick={toggleFullscreen} className="p-2 text-white/70 hover:text-white hover:bg-white/10 rounded-lg transition-colors" title="Fullscreen (F)">
      ...
      <div className="rounded-xl border bg-card/60 px-5 py-3">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className={cn("text-xs font-semibold tabular-nums", quizUnlocked ? "text-emerald-600" : "text-foreground")}>
            {watchedPercent}% <span className="text-muted-foreground font-normal">/ {QUIZ_UNLOCK_THRESHOLD}% required</span>
          </span>
        </div>
        <div className="h-2 bg-muted rounded-full overflow-hidden">
          <div
            className={cn("h-full rounded-full transition-all duration-500", quizUnlocked ? "bg-emerald-500" : "bg-primary")}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        ...
        <div className="lg:col-span-3 rounded-2xl border bg-card p-5 space-y-4">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground mb-1">About this lesson</p>
            <h2 className="font-semibold text-foreground text-base leading-snug">{partData?.title}</h2>
            <p className="text-sm text-muted-foreground mt-2 leading-relaxed">
              {partData?.description || "Work through this lesson at your own pace. Complete at least 90% of the video to unlock the quiz and test your knowledge."}
            </p>
          </div>

          <div>
            <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground mb-2">Keyboard shortcuts</p>
            <div className="flex flex-wrap gap-2">
              {[["Space / K","Play/Pause"],["← →","Skip 10s"],["M","Mute"],["F","Fullscreen"]].map(([k, l]) => (
                <div key={k} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-muted/60 border border-border/50">
        ...
        <div className={cn(
          ...
          <div className="relative">
            <svg width="100" height="100" viewBox="0 0 100 100" className="-rotate-90">
              <circle cx="50" cy="50" r={RING_R} fill="none" strokeWidth="6" stroke="currentColor" className="text-muted/30" />
              <circle
                cx="50" cy="50" r={RING_R} fill="none" strokeWidth="6"
          ...
          <div className="text-center space-y-1">
            <p className={cn("text-base font-bold", quizUnlocked ? "text-emerald-600" : "text-foreground")}>

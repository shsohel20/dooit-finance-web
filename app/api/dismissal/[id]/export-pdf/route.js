...
/**
 * Streams the alert dismissal PDF from the API with the caller's bearer token
 * attached.
 *
 ...
 * Same shape as the ECDD / SMR / GFS proxies.
 */
export async function GET(request, { params }) {
  ...
  const response = await fetch(`${BASE_URL}dismissal-report/${id}/export-pdf`, {
  ...
  return new NextResponse(buffer, {
    ...
    headers: {
      ...
      "Content-Disposition":
        response.headers.get("Content-Disposition") ?? `attachment; filename=DISMISSAL-${id}.pdf`,
    },
  });
}

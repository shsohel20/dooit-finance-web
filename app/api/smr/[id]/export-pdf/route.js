...
/**
 * Streams the SMR PDF from the API with the caller's bearer token attached.
 *
...
export async function GET(request, { params }) {
  ...
  const response = await fetch(`${BASE_URL}smr-report/${id}/export-pdf`, {
  ...
  return new NextResponse(buffer, {
    ...
    headers: {
      ...
      "Content-Disposition":
        response.headers.get("Content-Disposition") ?? `attachment; filename=SMR-${id}.pdf`,
    },
  });
}
